...
function PhotoUploadField({ value, existingUrl, onChange }) {
...
export default function EditTeacher() {
    ...
    api.get(`/teachers/${id}`)
      .then(res => {
        ...
        setForm({
          ...
          joiningDate:    fmt(t.joiningDate),
          employmentType: t.employmentType || '',
          currentStreet:  t.address?.street   || '',
          currentCity:    t.address?.city     || '',
          currentState:   t.address?.state    || '',
          currentPincode: t.address?.pincode  || '',
          permanentStreet:  t.permanentAddress?.street   || '',
          permanentCity:    t.permanentAddress?.city     || '',
          permanentState:   t.permanentAddress?.state    || '',
          permanentPincode: t.permanentAddress?.pincode  || '',
        ...
        if (t.photo) {
          ...
          setExistingPhotoUrl(t.photo.startsWith('http') ? t.photo : `${base}${t.photo}`);
        }
      })
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (noProfile) {
        // id is a User._id — create a Teacher profile attached to this existing user
        const res = await api.post('/teachers', { ...payload, userId: id });
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 fade-in">
      <PageHeader
        title="Edit Teacher"
        subtitle="Update teacher profile information"
        action={
          <button onClick={() => navigate(`/admin/teachers/${id}`)} className="btn-secondary flex items-center gap-2">
      ...
      <div className="card space-y-5">
        {step === 0 && (
          <>
            <PhotoUploadField value={photo} existingUrl={existingPhotoUrl} onChange={setPhoto} />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field label="New Password">
                <input className="input-field" type="password" value={form.password} onChange={e => set('password', e.target.value)} placeholder="Leave blank to keep current" />
              </Field>
              ...
              <Field label="Aadhar Number">
                <input className="input-field" value={form.aadharNo} onChange={e => set('aadharNo', e.target.value)} maxLength={12} />
              </Field>
            </div>
          </>
        )}
        ...
        {step === 1 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Employee ID" required>
              <input className="input-field" value={form.employeeId} onChange={e => set('employeeId', e.target.value)} />
            </Field>
            ...
            <Field label="Qualification">
              <input className="input-field" value={form.qualification} onChange={e => set('qualification', e.target.value)} />
            </Field>
            <Field label="Experience (years)">
              <input className="input-field" type="number" min="0" value={form.experience} onChange={e => set('experience', e.target.value)} />
            </Field>
          ...
          <>
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {['Street','City','State','Pincode'].map(f => (
                  <Field key={f} label={f}>
                    <input className="input-field" value={form[`current${f}`]} onChange={e => set(`current${f}`, e.target.value)} />
                  </Field>
                ))}
              </div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button type="button" onClick={copyAddress} className="text-xs text-emerald-600 hover:underline font-medium">Same as current</button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {['Street','City','State','Pincode'].map(f => (
                  <Field key={f} label={f}>
                    <input className="input-field" value={form[`permanent${f}`]} onChange={e => set(`permanent${f}`, e.target.value)} />
                  </Field>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Field label="Name"><input className="input-field" value={form.emergencyName} onChange={e => set('emergencyName', e.target.value)} /></Field>
                <Field label="Relation"><input className="input-field" value={form.emergencyRelation} onChange={e => set('emergencyRelation', e.target.value)} /></Field>
                <Field label="Phone"><input className="input-field" value={form.emergencyPhone} onChange={e => set('emergencyPhone', e.target.value)} /></Field>
              </div>
            </div>
          </>
        )}
        ...
        <div className="flex justify-between pt-2 border-t border-gray-100">
          <button onClick={() => step > 0 ? setStep(s => s - 1) : navigate(`/admin/teachers/${id}`)} className="btn-secondary flex items-center gap-2">
            <ChevronLeft className="w-4 h-4" />{step === 0 ? 'Cancel' : 'Back'}
          </button>
          {step < STEPS.length - 1
            ? <button onClick={() => setStep(s => s + 1)} className="btn-primary flex items-center gap-2">Next <ChevronRight className="w-4 h-4" /></button>
