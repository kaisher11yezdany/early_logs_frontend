...
export default function AddStudent() {
  ...
  const onChange = e => {
    ...
    setForm(f => {
      ...
      // Mirror addresses if sameAddress is checked
  ...
  const validateStep = (s) => {
    ...
    if (s === 1) {
      if (!form.name) { toast.error('Student name is required'); return false; }
    }
    if (s === 2) {
      if (!form.email) { toast.error('Email is required for login'); return false; }
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        phone: form.phone, password: form.password || form.admissionNo,
        // Student core
        classId: form.classId, admissionNo: form.admissionNo,
        rollNo: form.rollNo, academicYear: form.academicYear,
        admissionDate: form.admissionDate || undefined,
        admissionTime: form.admissionTime, admissionDay: form.admissionDay,
        // Identity
        dateOfBirth: form.dateOfBirth || undefined,
        gender: form.gender, caste: form.caste, category: form.category,
        nationality: form.nationality, religion: form.religion,
        placeOfBirth: form.placeOfBirth, aadharNo: form.aadharNo,
        language: form.language, bloodGroup: form.bloodGroup,
        // Addresses
        address: {
        ...
        parentInfo: {
          father: {
            name: form.fatherName, qualification: form.fatherQualification,
            occupation: form.fatherOccupation, aadharNo: form.fatherAadhar,
            email: form.fatherEmail, phone: form.fatherPhone
          },
          mother: {
        ...
        // Previous school
        previousSchool: {
          name: form.prevSchoolName, standardLastStudied: form.prevStandard,
          transferNoDate: form.prevTransferNoDate, previousProgress: form.prevProgress,
          dateOfLeaving: form.prevDateOfLeaving || undefined,
          tcNoDate: form.prevTcNoDate, penNo: form.prevPenNo,
          satsNo: form.prevSatsNo, apparId: form.prevApparId,
          udiseCode: form.prevUdiseCode
        },
        // Documents
        documents: {
          aadharCard: form.docAadhar, tc: form.docTc,
          birthCertificate: form.docBirth, bplCard: form.docBpl,
          casteIncomeCert: form.docCaste, conductCharacterCert: form.docConduct
        }
      };
  ...
  return (
    <div className="max-w-5xl mx-auto fade-in">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-gray-800">Enroll New Student</h1>
          <p className="text-sm text-gray-400 mt-0.5">2025-2026 Application Form · Early Foundation International School</p>
        </div>
      </div>
          ...
          <div className="card">
            ...
            {step === 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Class to Enroll" required>
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Field label="Phone Number">
                    <Input name="phone" form={form} onChange={onChange} placeholder="10-digit mobile number" />
                  </Field>
                  <Field label="Login Password">
                    <Input name="password" form={form} onChange={onChange} type="password" placeholder="Leave blank to use admission no" />
                  </Field>
                </div>
            ...
            {step === 3 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Occupation">
                  <Input name="fatherOccupation" form={form} onChange={onChange} placeholder="e.g. Engineer, Farmer" />
                </Field>
                <Field label="Aadhar Card No">
                  <Input name="fatherAadhar" form={form} onChange={onChange} placeholder="12-digit Aadhar number" />
                </Field>
                <Field label="Contact No">
                  <Input name="fatherPhone" form={form} onChange={onChange} placeholder="10-digit mobile number" />
                </Field>
                <div className="sm:col-span-2">
            ...
            {step === 4 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Occupation">
                  <Input name="motherOccupation" form={form} onChange={onChange} placeholder="e.g. Homemaker, Teacher" />
                </Field>
                <Field label="Aadhar Card No">
                  <Input name="motherAadhar" form={form} onChange={onChange} placeholder="12-digit Aadhar number" />
                </Field>
                <Field label="Contact No">
                  <Input name="motherPhone" form={form} onChange={onChange} placeholder="10-digit mobile number" />
                </Field>
                <div className="sm:col-span-2">
                  <Field label="E-Mail ID">
            ...
            {step === 6 && (
              <div className="space-y-3">
                <p className="text-sm text-gray-500 mb-4">Check all documents that have been submitted at the time of enrollment.</p>
                {[
                ...
                <div className="mt-4 p-4 bg-blue-50 rounded-xl border border-blue-100">
                  ...
                  <div className="grid grid-cols-2 gap-1 text-xs text-blue-700">
                    ...
                    <span>Class:</span><span className="font-medium">{classes.find(c => c._id === form.classId)?.name || '—'}</span>
                    <span>Documents:</span>
            ...
            <div className="flex justify-between items-center mt-6 pt-5 border-t border-gray-100">
              <button
                onClick={step === 0 ? () => navigate('/admin/students') : goBack}
